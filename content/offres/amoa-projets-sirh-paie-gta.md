---
slug: amoa-projets-sirh-paie-gta
title: AMOA Projets SIRH/Paie/GTA
pillar: conseil-projets-sirh
variant: conseil-projet
---

# AMOA Projets SIRH, Paie et GTA

**Piloter votre projet du côté métier - pas seulement du côté projet.**

L'AMOA classique sait conduire un projet : ateliers, comités, plannings, livrables. Mais sur un projet SIRH ou Paie, **la complexité est métier avant d'être projet**. Une AMOA qui ne parle pas paie, laissera passer la rubrique mal paramétrée, l'interface comptable bancale, le scénario de bascule incomplet - autant de coûts cachés qui se révèlent après le démarrage.

GBA Connect porte l'AMOA de vos projets SIRH/Paie/GTA avec une **double exigence : maîtrise méthodologique et maîtrise métier**, quel que soit l'éditeur retenu.

## À qui s'adresse cette offre

- Vous lancez un projet de refonte SIRH, paie ou GTA et cherchez une AMOA capable de challenger l'intégrateur sur le fond
- Vous souhaitez vous concentrer sur le pilotage métier et déléguer la conduite opérationnelle du projet
- Vous avez déjà une équipe projet mais besoin d'une expertise paie pour valider les paramétrages et la recette
- Vous voulez sécuriser une bascule (go-live) sur un environnement à fort enjeu social ou financier

## Ce que nous livrons

- **Cadrage projet** : périmètre, gouvernance, planning, charge interne, plan de communication
- **Rédaction et arbitrage des spécifications fonctionnelles** : règles de gestion, cas particuliers, populations atypiques
- **Pilotage de l'intégrateur** : suivi des engagements, contrôle qualité des livrables, gestion des évolutions
- **Conception et exécution de la stratégie de recette** : plan de tests, jeux de données, scénarios métier, recette utilisateur
- **Préparation et conduite de la bascule** : checklist go-live, plan de repli, suivi des premières paies post-démarrage
- **Conduite du changement** auprès des équipes paie, RH, managers et salariés

## Notre approche

> *« Leur expérience combinée de la paie et des éditeurs a fait la différence tout au long de la migration. Les échanges avec l'éditeur étaient plus fluides, les choix de paramétrage étaient challengés lorsque c'était nécessaire et les impacts métier étaient systématiquement pris en compte avant les arbitrages. »*
>
> - *Resp. Paie, ETI industrielle multi-sites - mission réalisée par les associés-fondateurs avant la création de GBA Connect*

## Indicateurs de réussite

- **Go-live tenu** dans le calendrier et le budget
- **Première paie post-démarrage** sans rejet DSN et sans régularisation massive
- **Équipes utilisatrices opérationnelles** dès le démarrage, sans dépendance prolongée à l'intégrateur
- **Dossier de recette complet** et archivé, opposable à l'intégrateur

## Format & durée

Mission longue, généralement entre **6 et 18 mois** selon l'ampleur du projet. Régie ou forfait par phase (cadrage, conception, recette, bascule).

## Ce que nous ne faisons pas

GBA Connect n'est **pas intégrateur** et ne réalise pas le paramétrage de l'outil. Notre rôle est de piloter, spécifier, contrôler et recetter - en représentation de votre intérêt métier, distinct de celui de l'intégrateur.

## Pour aller plus loin

→ Parlons de votre projet · Voir aussi : *Accompagnement de migration ADP* · *Reprise de données paie* · *Recette fonctionnelle paie*
